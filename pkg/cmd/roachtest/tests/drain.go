...
func runEarlyExitInConnectionWait(ctx context.Context, t test.Test, c cluster.Cluster) {
	...
	if _, err := db.Conn(ctx); err != nil {
		if !testutils.IsError(err, "server is not accepting clients, try another node") {
			require.FailNowf(t, "expected \"server is not accepting clients, try another node\"", "the actual error was %v", err)
		}
	} else {
